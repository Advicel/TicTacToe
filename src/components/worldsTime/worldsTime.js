...
export default function WorldsTime() {
  ...
  return (
    <div className={classes.worldsTime}>
      <button className={classes.button} onClick={() => handleClick('samara')}>
        время в Самаре
      </button>
      <button className={classes.button} onClick={() => handleClick('moscow')}>
        время в Москве
      </button>
      <div>{time === null ? 'здесь будет время' : time}</div>
    </div>
  );
}
